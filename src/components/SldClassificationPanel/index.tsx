...
export const SldClassificationPanel: React.FC<SldClassificationPanelProps> = ({
  ...
  const getSld = useCallback(async () => {
    // TODO Check if legend in tree is updated accordingly
    // TODO What about the legend in the pdf export?
    // Make url configurable by layer property
    // 1. this allows to use a static sld file for external layers
    // 2. for internal layer we use a proxy to get the sld from geoserver rest (as soon as useBearerToken is present? -> no)
  ...
  const getScaleDenominatorText = (scaleDenominator: ScaleDenominator): string => {
    ...
    if (minValue && !_isNil(maxValue)) {
      ...
      return t('SldClassificationPanel.scaleDenominatorText4', {
        max: maxValue
      });
    } else {
      return '';
  ...
  const btnProps: Partial<SimpleButtonProps> = {
    size: 'small',
    ...
    >
      {
        rules.map((rule, idx) => (
          ...
          >
            ...
            />
            {
              // TODO include full styler here as well?
            }
            <SLDRenderer
      ...
      <div className="filter-buttons">
        <SimpleButton
          ...
          tooltip={t('SldClassificationPanel.applyButtonTooltip')}
        ...
        <SimpleButton
          ...
          tooltip={t('SldClassificationPanel.restoreButtonTooltip')}
